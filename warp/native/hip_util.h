...
// hipStreamAddCaptureDependencies, so every call that means "replace the
// dependency set" appended to it instead.
...
// Warp 1.15 device-attribute, pointer-attribute and graph-memory shims
...
// HIP has these, and mapping them to the default flag silently turns an
// external event into an ordinary captured node: two graphs meant to
// synchronize through it then do not, and the second graph reads results the
...
#ifdef hipEventWaitExternal
#define CU_EVENT_WAIT_EXTERNAL hipEventWaitExternal
#else
#define CU_EVENT_WAIT_EXTERNAL 0
#endif
#endif  // CU_EVENT_WAIT_EXTERNAL
#ifndef CU_IPC_MEM_LAZY_ENABLE_PEER_ACCESS
#ifdef hipIpcMemLazyEnablePeerAccess
...
// Warp 1.15 driver-API wrapper types
using CUoccupancyB2DSize = size_t (*)(int block_size);
using CUlaunchConfig = hipLaunchConfig_t;
using CUmipmappedArray = hipMipmappedArray_t;
using CUmemorytype = hipMemoryType;
using cudaMemAllocNodeParams = hipMemAllocNodeParams;
...
#ifndef CU_MEMORYTYPE_UNIFIED
#define CU_MEMORYTYPE_UNIFIED hipMemoryTypeUnified
#endif  // CU_MEMORYTYPE_UNIFIED

// CUDA Runtime API -> HIP runtime API surface aliases
// Used by v1.12.1's wp_texture_array_create_surface_device etc.
#ifndef cudaResourceDesc
